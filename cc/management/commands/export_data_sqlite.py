...
class Command(BaseCommand):
        ...
        with connections['default'].cursor() as django_cursor:
            ...
            if session:
                ...
                with sqlite3.connect(sqlite_db_path) as conn:
                    ...
                    cursor.executemany(f'INSERT INTO cc_session ({", ".join(columns)}) VALUES ({placeholders})',
                                       [session])
                    conn.commit()
    ...
    def get_session_related_data(self, django_cursor, session_id, conn):
        ...
        cursor = conn.cursor()
        cursor.executemany(
            f'INSERT INTO {time_keeper_table_name} ({", ".join(time_keeper_columns)}) VALUES ({time_keeper_placeholders})',
            time_keepers)
        conn.commit()
        ...
        if annotation_folders:
            cursor.executemany(
                f'INSERT INTO {annotation_folder_table_name} ({", ".join(annotation_folder_columns)}) VALUES ({annotation_folder_placeholders})',
                annotation_folders)
            conn.commit()
            # Export Annotations associated to AnnotationFolders
            annotation_table_name = Annotation._meta.db_table
            django_cursor.execute(
                f'SELECT * FROM {annotation_table_name} WHERE folder_id IN ({", ".join(["%s"] * len(annotation_folders))})',
                [folder[0] for folder in annotation_folders])
            ...
            if annotations:
                for annotation in annotations:
                    ...
                    annotation[annotation_columns.index('session_id')] = None  # Clear session_id
                    cursor.executemany(
                        f'INSERT INTO {annotation_table_name} ({", ".join(annotation_columns)}) VALUES ({annotation_placeholders})',
                        [annotation])
                conn.commit()
            ...
            if isinstance(f, models.ManyToManyField):
                ...
                if related_rows:
                    for related_row in related_rows:
                        related_row = list(related_row)
                        related_row[related_columns.index('session_id')] = None  # Clear session_id
                        cursor.executemany(
                            f'INSERT INTO {related_table} ({", ".join(related_columns)}) VALUES ({related_placeholders})',
                            [related_row])
                    conn.commit()
                    if f.name == 'protocols':
                        ...
                        for related_row in related_rows:
                            ...
                            django_cursor.execute(f'SELECT * FROM {protocol_step_table_name} WHERE protocol_id = %s',
                                                  [related_row[position_of_protocol]])
                            ...
                            cursor.executemany(
                                f'INSERT INTO {protocol_step_table_name} ({", ".join(protocol_step_columns)}) VALUES ({protocol_step_placeholders})',
                                protocol_steps)
                            # Export ProtocolSection of this Protocol
                            protocol_section_table_name = ProtocolSection._meta.db_table
                            django_cursor.execute(f'SELECT * FROM {protocol_section_table_name} WHERE protocol_id = %s',
                                                  [related_row[position_of_protocol]])
                            ...
                            cursor.executemany(
                                f'INSERT INTO {protocol_section_table_name} ({", ".join(protocol_section_columns)}) VALUES ({protocol_section_placeholders})',
                                protocol_sections)
                            # Export StepVariation of these ProtocolSteps
                            step_variation_table_name = StepVariation._meta.db_table
                            django_cursor.execute(
                                f'SELECT * FROM {step_variation_table_name} WHERE step_id IN ({", ".join(["%s"] * len(protocol_steps))})',
                                [step[0] for step in protocol_steps])
                            ...
                            cursor.executemany(
                                f'INSERT INTO {step_variation_table_name} ({", ".join(step_variation_columns)}) VALUES ({step_variation_placeholders})',
                                step_variations)
                            conn.commit()
                            # Export Annotation associated to ProtocolStep
                            annotation_table_name = Annotation._meta.db_table
                            django_cursor.execute(
                                f'SELECT * FROM {annotation_table_name} WHERE step_id IN ({", ".join(["%s"] * len(protocol_steps))})',
                                [step[0] for step in protocol_steps])
                            ...
                            if step_annotations:
                                for step_annotation in step_annotations:
                                    step_annotation = list(step_annotation)
                                    step_annotation[annotation_columns.index('session_id')] = None  # Clear session_id
                                    cursor.executemany(
                                        f'INSERT INTO {annotation_table_name} ({", ".join(annotation_columns)}) VALUES ({annotation_placeholders})',
                                        [step_annotation])
                                conn.commit()
                            ...
                            django_cursor.execute(f'SELECT * FROM {protocol_reagent_table_name} WHERE protocol_id = %s',
                                                  [related_row[position_of_protocol]])
                            ...
                            if protocol_reagents:
                                cursor.executemany(
                                    f'INSERT INTO {protocol_reagent_table_name} ({", ".join(protocol_reagent_columns)}) VALUES ({protocol_reagent_placeholders})',
                                    protocol_reagents)
                                conn.commit()
                                # Export StepReagent associated to ProtocolReagent
                                step_reagent_table_name = StepReagent._meta.db_table
                                django_cursor.execute(
                                    f'SELECT * FROM {step_reagent_table_name} WHERE reagent_id IN ({", ".join(["%s"] * len(protocol_reagents))})',
                                    [reagent[0] for reagent in protocol_reagents])
                                ...
                                if step_reagents:
                                    cursor.executemany(
                                        f'INSERT INTO {step_reagent_table_name} ({", ".join(step_reagent_columns)}) VALUES ({step_reagent_placeholders})',
                                        step_reagents)
                                    conn.commit()
                                    # Export Reagent associated to ProtocolReagent
                                    reagent_table_name = Reagent._meta.db_table
                                    django_cursor.execute(
                                        f'SELECT * FROM {reagent_table_name} WHERE id IN ({", ".join(["%s"] * len(step_reagents))})',
                                        [reagent[1] for reagent in step_reagents])
                                    ...
                                    if reagents:
                                        cursor.executemany(
                                            f'INSERT INTO {reagent_table_name} ({", ".join(reagent_columns)}) VALUES ({reagent_placeholders})',
                                            reagents)
                                        conn.commit()
                                        # Export StoredReagent associated to Reagent
                                        stored_reagent_table_name = StoredReagent._meta.db_table
                                        django_cursor.execute(
                                            f'SELECT * FROM {stored_reagent_table_name} WHERE reagent_id IN ({", ".join(["%s"] * len(reagents))})',
                                            [reagent[0] for reagent in reagents])
                                        ...
                                        if stored_reagents:
                                            cursor.executemany(
                                                f'INSERT INTO {stored_reagent_table_name} ({", ".join(stored_reagent_columns)}) VALUES ({stored_reagent_placeholders})',
                                                stored_reagents)
                                            conn.commit()

                                            stored_reagent_orm = StoredReagent.objects.filter(
                                                id__in=[st[0] for st in stored_reagents])
                                            ...
                                            for st in stored_reagent_orm:
                                                ...
                                                if storage_object_list:
                                                    ...
                                                    django_cursor.execute(
                                                        f'SELECT * FROM {storage_object_table_name} WHERE id IN ({", ".join(["%s"] * len(storage_object_list))})',
                                                        storage_object_list)
                                                    storage_objects = django_cursor.fetchall()
                                                    storage_object_columns = [col[0] for col in
                                                                              django_cursor.description]
                                                    storage_object_placeholders = ', '.join(
                                                        ['?'] * len(storage_object_columns))
                                                    cursor.executemany(
                                                        f'INSERT INTO {storage_object_table_name} ({", ".join(storage_object_columns)}) VALUES ({storage_object_placeholders})',
                                                        storage_objects)
                                                    conn.commit()
                                                    # Export MetadataColumns associated to StoredReagent
                                                    metadata_column_table_name = MetadataColumn._meta.db_table
                                                    django_cursor.execute(
                                                        f'SELECT * FROM {metadata_column_table_name} WHERE stored_reagent_id IN ({", ".join(["%s"] * len(stored_reagents))})',
                                                        [st[0] for st in stored_reagents])
                                                    metadata_columns = django_cursor.fetchall()
                                                    metadata_column_columns = [col[0] for col in
                                                                               django_cursor.description]
                                                    metadata_columns_placeholders = ', '.join(
                                                        ['?'] * len(metadata_column_columns))
                                    ...
                                    django_cursor.execute(
                                        f'SELECT * FROM {reagent_action_table_name} WHERE step_reagent_id IN ({", ".join(["%s"] * len(step_reagents))}) AND session_id = %s',
                                        [reagent[0] for reagent in step_reagents] + [session_id])
                                    ...
                                    cursor.executemany(
                                        f'INSERT INTO {reagent_action_table_name} ({", ".join(reagent_action_columns)}) VALUES ({reagent_action_placeholders})',
                                        reagent_actions)
                                    conn.commit()
                        ...
                        django_cursor.execute(
                            f'SELECT * FROM {protocol_table_name} WHERE id IN ({", ".join(["%s"] * len(protocol_ids))})',
                            protocol_ids)
                        ...
                        if protocols:
                            cursor.executemany(
                                f'INSERT INTO {protocol_table_name} ({", ".join(protocol_columns)}) VALUES ({protocol_placeholders})',
                                protocols)
                            conn.commit()
                            # Export ProtocolRating associated to Protocol
                            protocol_rating_table_name = ProtocolRating._meta.db_table
                            django_cursor.execute(
                                f'SELECT * FROM {protocol_rating_table_name} WHERE protocol_id IN ({", ".join(["%s"] * len(protocol_ids))})',
                                protocol_ids)
                            ...
                            if protocol_ratings:
                                cursor.executemany(
                                    f'INSERT INTO {protocol_rating_table_name} ({", ".join(protocol_rating_columns)}) VALUES ({protocol_rating_placeholders})',
                                    protocol_ratings)
                                conn.commit()
        if annotations:
            ...
            django_cursor.execute(
                f'SELECT * FROM {instrument_usage_table_name} WHERE annotation_id IN ({", ".join(["%s"] * len(annotations))})',
                [annotation[0] for annotation in annotations])
            ...
            if instrument_usages:
                cursor.executemany(
                    f'INSERT INTO {instrument_usage_table_name} ({", ".join(instrument_usage_columns)}) VALUES ({instrument_usage_placeholders})',
                    instrument_usages)
                conn.commit()
                # Export Instrument associated to InstrumentUsage
                instrument_table_name = Instrument._meta.db_table
                instrument_ids = list(set([instrument_usage[6] for instrument_usage in instrument_usages]))
                django_cursor.execute(
                    f'SELECT * FROM {instrument_table_name} WHERE id IN ({", ".join(["%s"] * len(instrument_ids))})',
                    instrument_ids)
                ...
                if instruments:
                    ...
                    metadata_column_table_name = MetadataColumn._meta.db_table
                    django_cursor.execute(
                        f'SELECT * FROM {metadata_column_table_name} WHERE instrument_id IN ({", ".join(["%s"] * len(instruments))})',
            ...
            metadata_column_table_name = MetadataColumn._meta.db_table
            django_cursor.execute(
                f'SELECT * FROM {metadata_column_table_name} WHERE annotation_id IN ({", ".join(["%s"] * len(annotations))})',
                [annotation[0] for annotation in annotations])
            metadata_columns = django_cursor.fetchall()
        ...
        if metadata_columns:
            ...
            metadata_column_table_name = MetadataColumn._meta.db_table
            cursor.executemany(
                f'INSERT INTO {metadata_column_table_name} ({", ".join(metadata_column_columns)}) VALUES ({metadata_columns_placeholders})',
