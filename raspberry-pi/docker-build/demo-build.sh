...
# Demo CUPCAKE Pi 5 Docker Build 
# Shows complete build process with working base image

set -e

echo "=== CUPCAKE Pi 5 Docker Build Demo ==="
...
# Fix the known working base image URL
log "Creating demo build with known working Raspberry Pi OS image..."

# Create a minimal demo build script 
cat > build-scripts/demo-build-cupcake.sh << 'EOF'
#!/bin/bash
...
chmod +x build-scripts/demo-build-cupcake.sh

# Run the demo build
log "Starting Docker demo build..."

docker run \
    --name cupcake-demo-build \
    --rm \
    --privileged \
    --cap-add=SYS_ADMIN \
    ...
    cupcake-pi5-builder \
    /build/scripts/demo-build-cupcake.sh
...
if [ -d "output" ] && [ "$(ls -A output 2>/dev/null)" ]; then
    ...
    echo "  • Image creation and partitioning"  
    echo "  • File system management"
    echo "  • CUPCAKE structure creation"
    echo "  • Configuration generation"
    echo ""
    echo "🚀 For production build:"
    echo "  • Fix Raspberry Pi OS download URL"
    echo "  • Run full build process (30-90 minutes)"
    echo "  • Get complete 6-8GB CUPCAKE image"
else
    echo "❌ Demo build failed - no output files"
fi